...

{
	...
	if (_nums > 0) then {
		private _curar = if (_x select 1 == 0) then {_wp_array_inf} else {_wp_array_vecs};
...

{
	private _nums = _x select 2;
	if (_nums > 0) then {
		private _curar = if (_x select 1 == 0) then {_wp_array_pat_inf} else {_wp_array_pat_vecs};
		for "_xxx" from 1 to _nums do {
			private _wp_ran = (count _curar) call d_fnc_RandomFloor;
			[_x select 0, [_curar select _wp_ran], _trg_center, _x select 1, (if ((_x select 0) == "allmen" || {(_x select 0) == "specops"}) then {"patrol2mt"} else {"patrol"}), d_enemy_side_short, 0, -1.111, 1, [_trg_center, _patrol_radius]] call d_fnc_makegroup;
			_curar deleteAt _wp_ran;
			sleep 0.4;
		};
	};
