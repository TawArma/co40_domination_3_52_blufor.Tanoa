...
private _r = if (toUpper(getText(configFile>>"CfgAmmo">>(param [4])>>"simulation")) in d_hd_sim_types) then {
	param [2]
} else {
	if (d_MTTowerSatchelsOnly == 1 && {getText(configFile>>"CfgAmmo">>(param [4])>>"CraterEffects") == "BombCrater"}) then {
		param [2]
	} else {
		0
	};
