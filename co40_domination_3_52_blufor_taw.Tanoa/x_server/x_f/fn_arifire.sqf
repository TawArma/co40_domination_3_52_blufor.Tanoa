...
_logic1 kbTell [_aop,_topicside_arti,"ArtilleryRoger",["1","",localize "STR_DOM_MISSIONSTRING_934",[]],"SIDE"];
sleep 1;
private _aristr = localize "STR_DOM_MISSIONSTRING_937";
_logic1 kbTell [_logic, _topicside, "ArtilleryUnAvailable", ["1", "", _aristr, []],"SIDE"];

sleep 6.54;
private _aop = missionNamespace getVariable _arti_operator;
if (isNil "_aop" || {isNull _aop}) then {_aop = _logic};
_logic1 kbTell [_aop, _topicside_arti,"ArtilleryExecute", ["1", "", _aristr, []], ["2", "", getText(configFile>>"CfgMagazines">>_ari_type>>"displayname"), []], ["3", "", str _ari_salvos, []], "SIDE"];
...
for "_series" from 1 to _ari_salvos do {
	...
	private _aop = missionNamespace getVariable _arti_operator;
	if (isNil "_aop" || {isNull _aop}) then {_aop = _logic};
	_logic1 kbTell [_aop,_topicside_arti,"ArtilleryOTW",["1","",str _series,[]],["2","",str(round _eta_time),[]],"SIDE"];

	sleep _eta_time;
	
	private _aop = missionNamespace getVariable _arti_operator;
	if (isNil "_aop" || {isNull _aop}) then {_aop = _logic};
	_logic1 kbTell [_aop,_topicside_arti,"ArtillerySplash",["1","",str _series,[]],"SIDE"];

	if (!_is_flare && {!_is_smoke}) then {
		{
			_enemy_units pushBackUnique _x;
		} forEach (_ari_tgt_pos nearEntities [_soldier_type, 40]);
	};

	if (_series < _ari_salvos) then {
		private _aop = missionNamespace getVariable _arti_operator;
		if (isNil "_aop" || {isNull _aop}) then {_aop = _logic};
		_logic1 kbTell [_aop,_topicside_arti,"ArtilleryReload",["1","",_aristr,[]],"SIDE"];
		sleep _reloadtime;
	};
};
...
if (!isNil "_aop" && {!isNull _aop}) then {
	_aop addScore ({!alive _x} count _enemy_units);
#ifdef __TT__
	if (d_cur_tgt_pos distance2D _ari_tgt_pos < 500) then {
		if (_side_arti_op == opfor) then {
			{
				if (!alive _x) then {
...
private _aop = missionNamespace getVariable _arti_operator;
if (isNil "_aop" || {isNull _aop}) then {_aop = _logic};
_logic1 kbTell [_aop,_topicside_arti,"ArtilleryComplete",["1","",_aristr,[]],"SIDE"];
...
[_ari_salvos, _aristr, _logic1, _side_arti_op] spawn {
	...
	(param [2]) kbTell [_logic, _topicside, "ArtilleryAvailable", ["1", "", _aristr, []],"SIDE"];
#else
	(param [2]) kbTell [d_kb_logic2, d_kb_topic_side, "ArtilleryAvailable", ["1", "", _aristr, []],"SIDE"];
#endif
};
