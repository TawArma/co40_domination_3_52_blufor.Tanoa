...
if ({alive _x} count units (group player) > 0) then {
	if (isNil "_this" || {_this isEqualTo []}) then {
		...
		_pos_p set [2,0];
		{if (!isPlayer _x && {isNull objectParent _x} && {_x distance2D _pos_p > 500}) then {_x setPos _pos_p}} forEach (formationMembers player);
	} else {
		...
		{
			if (!isPlayer _x && {isNull objectParent _x} && {_x distance2D _pos_p > 500}) then {
