...
if (!isNull _killer && {isPlayer _killer} && {vehicle _killer != vehicle _killed}) then {
	...
	if (!_killedfriendly) then {
		...
		private _lstr = if (d_WithRevive == 0) then {"STR_DOM_MISSIONSTRING_514"} else {"STR_DOM_MISSIONSTRING_515"};
		format [localize _lstr, _namek, _namep, "WEST", d_tt_points select 8] remoteExecCall ["sideChat", [0, -2] select isDedicated];
	} else {
