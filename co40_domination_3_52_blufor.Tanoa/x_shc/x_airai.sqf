...
_pat_pos set [2, _cur_tgt_pos select 2]
	...
	__TRACE_1("","_xdist")
		...
		if (!isNull _curvec && {_curvec distance2D _cur_tgt_pos < _radius}) then {
			if (_type == "HAC" || {_type == "LAC"}) then {
				private _old_pat_pos = _pat_pos;
				__patternpos;
				while {_pat_pos distance2D _old_pat_pos < 100} do {
					__patternpos;
					sleep 0.01;
				};
