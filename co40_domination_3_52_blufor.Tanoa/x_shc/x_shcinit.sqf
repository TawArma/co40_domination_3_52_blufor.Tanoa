...
//call d_fnc_NotAliveRemover;
execFSM "fsms\fn_NotAliveRemover.fsm";
if (isServer) then {
	//call d_fnc_NotAliveRemoverUnits;
	execFSM "fsms\fn_NotAliveRemoverUnits.fsm";
};
...
if (d_MissionType != 2) then {
	0 spawn {
		...
		sleep 30;
		if (isServer && {!isNil "HC_D_UNIT"}) exitWith {};
		sleep 1200;
