...
if (_unit getVariable ["xr_ReviveAction", -9999] == -9999) then {
	private _id = [
		...
		/* 2 idle icon */					"a3\ui_f\data\revive\medikit_ca.paa",
		/* 3 progress icon */				"a3\ui_f\data\IGUI\Cfg\HoldActions\holdAction_reviveMedic_ca.paa",
		/* 4 condition to show */			"(_this distance2D _target <= 3) && {_target getVariable 'xr_pluncon'} && {!(_this getVariable 'xr_pisinaction')} && {!(_target getVariable 'xr_dragged')}",
	...
	_id = _unit addAction [format ["<t color='#FF0000'>%2 %1</t>", name _unit, localize "STR_DOM_MISSIONSTRING_1704"], {player setVariable ["xr_cursorTarget", param [0]]; 0 spawn xr_fnc_drag}, [], -2, false, false, "", "(_this distance2D _target <= 3) && {_target getVariable 'xr_pluncon'} && {!(_this getVariable 'xr_pisinaction')} && {!(_target getVariable 'xr_dragged')}"];
	__TRACE_1("id2","_id")
