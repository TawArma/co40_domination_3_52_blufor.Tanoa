...
while {!isNull objectParent player && {alive player} && {time < _endtime} && {!(player getVariable ["xr_pluncon", false])}} do {
	sleep 1;
	hintSilent _struct_text;
};
if (_vtype == "MHQ" && {!isNull objectParent player && {alive player}} && {!(player getVariable ["xr_pluncon", false])}) then {
