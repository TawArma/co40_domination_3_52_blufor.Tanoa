...
(_uav select 0) spawn {
	...
	while {true} do {
		while {!isNull (findDisplay 160) && {!isNull (getConnectedUav player)} && {alive player} && {!(player getVariable ["xr_pluncon", false])} && {alive _uav}} do {
			sleep 1.1;
		};
		if ((isNull (findDisplay 160) && {(UAVControl _uav) select 1 == ""}) || {!alive player} || {player getVariable ["xr_pluncon", false]} || {!alive _uav}) exitWith {};
		while {(UAVControl _uav) select 1 != "" && {alive player} && {!(player getVariable ["xr_pluncon", false])} && {alive _uav}} do {
			sleep 1.1;
		};
		if ((isNull (findDisplay 160) && {(UAVControl _uav) select 1 == ""}) || {!alive player} || {player getVariable ["xr_pluncon", false]} || {!alive _uav}) exitWith {};
	};
