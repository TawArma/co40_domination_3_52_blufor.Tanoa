...
if (isDedicated || {param [1] != player} || {!alive player} || {player getVariable ["xr_pluncon", false]}) exitWith {};
...
if (d_with_ranked) then {
	[param [0], name player, d_ranked_a select 7] remoteExecCall ["d_fnc_ampoi", 2];
};
