...
if (!d_show_pname_hud && {!visibleMap} && {isNil "d_is_sat_on"}) then {
	...
	if (alive _ct && {_ct isKindOf "CAManBase"} && {!(player getVariable ["xr_pluncon", false])} && {_ct != player} && {((positionCameraToWorld [0,0,0]) distance _ct) <= (d_dist_pname_hud / 2)} && {side (group _ct) getFriend d_player_side >= 0.6}) then { // && {isPlayer _ct}
		...
		private _name = [(name _ct) + d_phud_loc883, [getText(configFile>>"CfgVehicles">>typeOf _ct>>"displayName") ,name _ct] select (isPlayer _ct)] select !(_ct getVariable ["xr_pluncon", false]);
		...
		] select (getNumber(configFile>>"CfgVehicles">>typeOf player>>"attendant") != 1);
