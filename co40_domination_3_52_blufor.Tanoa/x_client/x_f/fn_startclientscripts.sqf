...
if (!(d_clientScriptsAr select 1) && {!isNil "d_player_autokick_time"}) then {
	if (isNil "d_nomercyendtime") then {
		...
		if (_vec != player && {_vec isKindOf "Air"}) then {
			...
			if ((toUpper(_type) in (d_mt_bonus_vehicle_array select _numside) || {toUpper(_type) in (d_sm_bonus_vehicle_array select _numside)}) && {(player == driver _vec || {player == gunner _vec} || {player == commander _vec})}) then {
#endif
