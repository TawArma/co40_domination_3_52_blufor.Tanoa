...
if (_d_vec < 400) exitWith {
	...
	if (!alive _vec) exitWith {};
	if (d_with_ai || {d_with_ai_features == 0} || {player getVariable ["d_is_engineer", false]}) then {
		__staticl;
	} else {
...
#else
	{if ((_x select 3) == _d_vec) exitWith {_car = _x}} forEach d_choppers_blufor;
#endif
	__TRACE_1("","_car")
	if !(_car isEqualTo []) then {
		if (!alive _vec) exitWith {};
...
if (_d_vec < 5000) exitWith {
	private _car = [];
	{if ((_x select 3) == _d_vec) exitWith {_car = _x}} forEach d_choppers_opfor;
	__TRACE_1("","_car")
	if !(_car isEqualTo []) then {
		if (!alive _vec) exitWith {};
		missionNamespace setVariable [_car select 0, _vec];
