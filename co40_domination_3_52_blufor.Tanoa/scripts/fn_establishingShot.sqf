...
if (isNil "BIS_fnc_establishingShot_skip") then {
	...
	[_pos, _alt, _rad, _ang, _dir] spawn {
		...
		while {isNil "BIS_fnc_establishingShot_skip" && {!( player getVariable ["xr_pluncon", false])} && {alive player}} do {
			_ang = _ang - 0.5;
			...
			waitUntil {camCommitted BIS_fnc_establishingShot_fakeUAV || {!isNil "BIS_fnc_establishingShot_skip"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
